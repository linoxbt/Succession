...
| M3 asset lifecycle; B12 relisting | Deployed and accepted | A refunded live identity was relisted under a new id and evaluator-settled; UI distinguishes memory scope from whole-token transfer. |
...
| B2 confirmation; M2 recovery; B8–B11, B13–B16 UI | Live EOA wallet flow accepted | Buyer approved USDC, funded escrow, authenticated after settlement, imported 49 records and recovered its certificate. Browser regressions still use controlled wallet fixtures; provider-popup coverage remains open. |
...
| P4 frontend loading | Verified locally | Landing renders without requesting wallet modules; app/wallet bundles load on entering the console. A large wallet-chunk build warning remains. |
...
not a mainnet or external-audit certification; live Virtuals ACP jobs, provider
popup acceptance, monitoring, capacity and deeper reorg reconciliation remain.
